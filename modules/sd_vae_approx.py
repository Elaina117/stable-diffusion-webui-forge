...
def model():
    if not shared.sd_model.is_wui_legacy_model():
        return None
    ...
    if loaded_model is None:
        ...
        if not os.path.exists(model_path):
            model_path = os.path.join(paths.models_path, "VAE-approx", model_name)
            download_model(model_path, 'https://github.com/AUTOMATIC1111/stable-diffusion-web'+'ui/releases/download/v1.0.0-pre/' + model_name)
